...
(function($){
	$.springrain={
			...
			mydelete:function(_id,_url,_tips,_redirect){
				...
				layer.confirm(_tips, {icon: 3,offset:['',''], title:'提示'}, function(index){
					  jQuery.ajax({
						  url:_url,
						  type:"post",
						  data:_pars,
			...
			mydeletemore:function(formId,_url,_tips,_redirect){
				...
				if (arr=="") {
					layer.confirm("没有选择待删除的数据", {icon: 3, title:'提示',offset:['','']}, function(index){
						  layer.close(index);
					});
					return false;
				}
				...
				_tips=_tips?_tips:'是否删除?';
				layer.confirm(_tips, {icon: 3, title:'提示',offset:['','']}, function(index){
					  jQuery.ajax({
						  url:_url,
						  type:"post",
						  data:{records:arr},
			...
			ajaxpostonlayer:function(pageurl,listurl,mydata,msg,callback){
				...
				if(pageurl==null||pageurl==''){
					layer.alert('提交地址不能为空！', {icon: 5,offset:['','']});
				...
				jQuery.ajax({
					...
					success:function(ret){
						...
						if(ret.status=="success"){
							layer.alert(msg, {icon: 1,offset:['','']},function(){
								layer.closeAll();
