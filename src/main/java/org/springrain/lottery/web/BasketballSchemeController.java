 ...
 */
@Controller
@RequestMapping(value="/basketballscheme")
public class BasketballSchemeController  extends BaseController {
	@Resource
	...
	public String list(HttpServletRequest request, Model model,BasketballScheme basketballScheme) 
		...
		if("1".equals(request.getParameter("k"))){
			...
			return  "/lottery/basketballorder/basketballorderList";
			...
			String zid2 = request.getParameter("zid");
			if(StringUtils.isBlank(zid2)){
				zid2=null;
			}
			...
			if(starttime=="0000-01-01" && endtime=="9999-01-01"){
				datas= basketballSchemeService.queryForList(new Finder("select a.*,c.nickname as membernickname,c.isinternal as isinternal  from basketball_scheme a LEFT JOIN bet_member c on c.id2=a.memberid2  where a.schemeid in (select schemeid from basketball_order_content where (:zid is null or zid = :zid) GROUP BY schemeid) and (:memberid2 is null or a.memberid2 = :memberid2)  and (:situation is null or a.situation = :situation) and (a.agentid = :agentid or a.agentparentids like :agentparentids)").setParam("agentid", pagentid).setParam("agentparentids", "%"+agentparentids+"%").setParam("zid", zid2).setParam("memberid2", memberid2).setParam("situation", situation),BasketballScheme.class,page);
				bettingmoneyTotal = basketballSchemeService.queryForObject(new Finder("select sum(a.bettingmoney) from basketball_scheme a LEFT JOIN basketball_league_playmethod b on a.playmethodid = b.id where a.schemeid in (select schemeid from basketball_order_content where (:zid is null or zid = :zid) GROUP BY schemeid) and  (:memberid2 is null or a.memberid2 = :memberid2)  and (:situation is null or a.situation = :situation) and (a.agentid = :agentid or a.agentparentids like :agentparentids)").setParam("zid", zid2).setParam("agentid", pagentid).setParam("agentparentids", "%"+agentparentids+"%").setParam("memberid2", memberid2).setParam("situation", situation),Double.class);
				bettingwinTotal = basketballSchemeService.queryForObject(new Finder("select sum(a.bettingwin) from basketball_scheme a LEFT JOIN basketball_league_playmethod b on a.playmethodid = b.id where a.schemeid in (select schemeid from basketball_order_content where (:zid is null or zid = :zid) GROUP BY schemeid) and  (:memberid2 is null or a.memberid2 = :memberid2)  and (:situation is null or a.situation = :situation) and (a.agentid = :agentid or a.agentparentids like :agentparentids)").setParam("zid", zid2).setParam("agentid", pagentid).setParam("agentparentids", "%"+agentparentids+"%").setParam("memberid2", memberid2).setParam("situation", situation),Double.class);
			}else{
				datas= basketballSchemeService.queryForList(new Finder("select a.*,c.nickname as membernickname,c.isinternal as isinternal  from basketball_scheme a LEFT JOIN bet_member c on c.id2=a.memberid2  where a.schemeid in (select schemeid from basketball_order_content where (:zid is null or zid = :zid) GROUP BY schemeid) and substr(a.bettingtime,1,10)>=:starttime and substr(a.bettingtime,1,10)<=:endtime and (:memberid2 is null or a.memberid2 = :memberid2) and (:situation is null or a.situation = :situation) and (a.agentid = :agentid or a.agentparentids like :agentparentids)").setParam("agentid", pagentid).setParam("agentparentids", "%"+agentparentids+"%").setParam("zid", zid2).setParam("starttime",startDate).setParam("endtime", endDate).setParam("memberid2", memberid2).setParam("situation", situation),BasketballScheme.class,page);
				bettingmoneyTotal = basketballSchemeService.queryForObject(new Finder("select sum(a.bettingmoney) from basketball_scheme a LEFT JOIN basketball_league_playmethod b on a.playmethodid = b.id where a.schemeid in (select schemeid from basketball_order_content where (:zid is null or zid = :zid) GROUP BY schemeid) and  substr(a.bettingtime,1,10)>=:starttime and substr(a.bettingtime,1,10)<=:endtime and (:memberid2 is null or a.memberid2 = :memberid2) and (:situation is null or a.situation = :situation) and (a.agentid = :agentid or a.agentparentids like :agentparentids)").setParam("zid", zid2).setParam("starttime",startDate).setParam("agentid", pagentid).setParam("agentparentids", "%"+agentparentids+"%").setParam("endtime", endDate).setParam("memberid2", memberid2).setParam("situation", situation),Double.class);
				bettingwinTotal = basketballSchemeService.queryForObject(new Finder("select sum(a.bettingwin) from basketball_scheme a LEFT JOIN basketball_league_playmethod b on a.playmethodid = b.id where a.schemeid in (select schemeid from basketball_order_content where (:zid is null or zid = :zid) GROUP BY schemeid) and substr(a.bettingtime,1,10)>=:starttime and substr(a.bettingtime,1,10)<=:endtime and (:memberid2 is null or a.memberid2 = :memberid2) and (:situation is null or a.situation = :situation) and (a.agentid = :agentid or a.agentparentids like :agentparentids)").setParam("zid", zid2).setParam("starttime",startDate).setParam("agentid", pagentid).setParam("agentparentids", "%"+agentparentids+"%").setParam("endtime", endDate).setParam("memberid2", memberid2).setParam("situation", situation),Double.class);
			}
	...
	ReturnDatas saveorupdate(Model model,BasketballScheme basketballScheme,HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		try {
			...
			if("1".equals(request.getParameter("k"))){
					...
					if(arrange.getStarttime().after(new Date())){
						...
						if(result!=null){
							if("--:--".equals(result.getScore())){
								...
								//同一环境所有平台都设置为无效比赛
								basketballOrderContentService.update(new Finder("update basketball_order_content set odds = 1,settletime=now(),result=1,resultname=:resultname where zid = :zid").setParam("zid", zid).setParam("resultname", "无效比赛"));
								...
								basketballResult.setState(2);//状态1.已确认 3.未确认 2:无效比赛
								basketballResult.setIssettle(1);//1已结算 3未结算
				...
				basketballSchemeService.update(new Finder("update basketball_scheme set statement=:statement where id=:id ").setParam("statement", "").setParam("id", id));
				...
				if(scheme!=null){
					...
					List<BasketballOrder> suestateList = basketballOrderService.queryForList(new Finder("select orderid from basketball_order where schemeid = :schemeid and issuestate = 3").setParam("schemeid", scheme.getSchemeid()),BasketballOrder.class);
					 ...
					 * 比赛开始后就不可以撤销
					 */
					List<BasketballOrderContent> contentDatas=basketballOrderContentService.queryForList(new Finder("select * from basketball_order_content where schemeid = :schemeid").setParam("schemeid", scheme.getSchemeid()),BasketballOrderContent.class);
					if(contentDatas!=null){
						for(BasketballOrderContent orderContent : contentDatas){
							BasketballLeagueArrange matchDatas = basketballLeagueArrangeService.queryForObject(new Finder("select * from basketball_league_arrange where mid = :mid").setParam("mid", orderContent.getMid()),BasketballLeagueArrange.class);
							if(matchDatas!=null){
								Date time = new Date();
					...
					if(scheme.getSituation()!=null&&scheme.getSituation()==0){
						...
						if(member!=null){
							basketballSchemeService.update(new Finder("update basketball_scheme set buytype=1,situation=:situation where id=:id").setParam("situation", 2).setParam("id", scheme.getId()));
