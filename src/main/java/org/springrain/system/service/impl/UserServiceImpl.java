...
import org.springrain.lottery.utils.AddressUtils;
 ...
 * @see org.springrain.Agent.service.impl.User
 */
@Service("userService")
...
//		.append(") as m where m.userId=u.id ");
	...
	public String deleteUserById(String userId) throws Exception {
		...
		super.deleteById(userId, User.class);
		
		return null;
	}
	@Override
	public void deleteUserByIds(List<String>ids,Class clazz) throws Exception{
	...
	public User findByAccount(String account) throws Exception {
		...
		User u= queryForObject(finder, User.class);
	...
	public void updateUserLogin(String ip, Date date,String account) throws UnsupportedEncodingException, Exception {
		...
		User user1=this.findByAccount(account);
		user1.setIp(ip+" "+address);
		user1.setLogintime(date);
		this.update(new Finder("update t_user set logintime=:logintime,ip=:ip where account=:account ").setParam("logintime", user1.getLogintime()).setParam("ip", user1.getIp()).setParam("account", account));
