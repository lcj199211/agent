 ...
 */
	...
	@Override
	@Async
	public void saveFwlog1(String userId,String ip, Fwlog fwlog) throws UnsupportedEncodingException, Exception {
		String address=AddressUtils.getAddresses("ip="+ip, "utf-8");
		fwlog.setIp(ip+" "+address);
		User user = userService.queryForObject(new Finder("select *from t_user where id=:id ").setParam("id", userId), User.class);
		fwlog.setUid(user.getId2());
		this.save(fwlog);
	}
